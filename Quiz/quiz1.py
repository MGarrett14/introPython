...
A; variable is a container for different values you put into a prograam
# 2. Name three (3) operators we've learned about in class and describe 
Three operators we've learned in class is mathmatic operator-symbols used to allow programmmers
to perform math operations, Assignment operator- symbols used to assian a variable to a value,
and Comparison operator- symbols used to compare values
# a situation where you would use each one. 
you would used the mathmatic operator when making a math problem, a assignment operator to assign
a variable like Numberofcarrots=22, you would use a comparion operator to see if a variable is larger
than the other like 24>2

# 3. Describe the difference between camal case and pascal case.
The difference between camal case and pascal case is that, camal case you capitalize every new word,
but the first one, and pascal case is the exact same except you can capitalize the first word
# 4. Which operator is being used in the example below
The operator that is being used in the example below is comparison operator
numberOfApples = 10
numberOfOranges = 12
...
GiftCardFromAmountFamilyMember= 100
GiftCardFromAmountAnotherFamilyMember= 50
GiftCardAmountFromFriend= 35.50
TotalAmountFromAllGiftCards= 185.50
...
FullNameofVisitors = input('What is your full name?')
AgeofVisitors = input('How old are you?')
AllergiesofVisitors
